...
function GameStore.processStackablePurchase(player, offerId, offerCount, offerName)
    ...
    if player:getFreeCapacity() < ItemType(offerId):getWeight(1) + ItemType(23782):getWeight() then
        return error({code = 0, message = "Please make sure you have free capacity to hold this item."})
    end
    elseif player:getFreeCapacity() < ItemType(offerId):getWeight(offerCount) + ItemType(23782):getWeight() then
        return error({code = 0, message = "Please make sure you have free capacity to hold this item."})
    end
